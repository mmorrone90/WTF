...
export default function TrendingCollections() {
  ...
  const trendingCollections = collections.filter(collection => collection.trending).slice(0, 5);
  ...
  const paginate = (newDirection: number) => {
    ...
    setCurrentIndex((prevIndex) => {
      ...
      if (nextIndex >= trendingCollections.length) return 0;
      if (nextIndex < 0) return trendingCollections.length - 1;
      return nextIndex;
    });
  };
  ...
  const handleViewDetails = () => {
    navigate(`/collection/${trendingCollections[currentIndex].id}`);
  };

  const currentCollection = trendingCollections[currentIndex];
